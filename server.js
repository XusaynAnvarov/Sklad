...
app.get("/api/img", async (req, res) => {
  try {
    const { default: img } = await import("./api/lib/imgproxy.js?v=20260830a");
...
setInterval(async () => {
  ...
  lastDayReport = day;
  try {
    const { dayRange, buildSummary, formatMessage } = await import("./api/admin/day-report.js?v=20260830a");
    const { sget } = await import("./api/lib/supa.js?v=20260830a");
