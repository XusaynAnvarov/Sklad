...
// ========================================================================
import { initCursorGlow, initTheme, initStarfield, makeThemeToggle } from "./effects.js?v=20260914b";
import { applyI18n, makeLangSwitcher } from "./i18n.js?v=20260914b";
import { iconSvg } from "./icons.js?v=20260914b";
import { thumb } from "./img.js?v=20260914b";
import { подходит } from "./productsearch.js?v=20260914b";
import { подписьКода } from "./catalogcode.js?v=20260914b";
...
async function loadItems() {
  ...
  const norm = (d) => ({ id: d.id, name: d.name, sku: d.sku || "", code: d.code || "", category: d.category, photo_url: d.photo_url, photos: (Array.isArray(d.photos) && d.photos.length) ? d.photos : (d.photo_url ? [d.photo_url] : []), status: d.status, is_new: !!d.is_new, hit: !!d.hit });
  if (useSupabase) {
  ...
  return (store.products || []).map(p => {
    ...
    return {
      id: p.id, name: p.name, sku: p.sku || "", code: p.code || "", category: p.category, photo_url: p.photo_url,
...
function cardHtml(p, i) {
  ...
  return `<div class="prod reveal" data-id="${id}" style="animation-delay:${(i % 12) * 0.03}s">
      ...
      <div class="body">
        ...
        ${подписьКода(p) ? `<div class="cat" style="font-size:11px;opacity:.8">${escapeHtml(подписьКода(p))}</div>` : ""}
...
function filteredItems() {
  ...
  return items.filter(p =>
    ...
    // название, категория, артикул и постоянный код — одно правило на весь склад
    подходит(p, q));
}
...
function wirePdfButton() {
  ...
  btn.addEventListener("click", async () => {
    ...
    btn.disabled = true;
    try {
      const m = await import("./catalog-pdf.js?v=20260914b");
