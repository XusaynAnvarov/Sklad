...
// ВАЖНО: «число» — это ячейка, состоящая ТОЛЬКО из числа (с пробелами-разделителями),
// а не любой текст с цифрой внутри. Иначе имена вроде «Челнок (790)», «Нож-100», «P36LN»
// ошибочно считались числами и строки пропускались.
const isNumeric = (v) => { const s = String(v == null ? "" : v).trim().replace(/\s/g, ""); return s !== "" && /^-?\d+([.,]\d+)?$/.test(s); };
