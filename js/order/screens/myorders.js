...
//  и его состояние.
//
//  Править заказ можно, пока мы его не собираем: и когда считаем цену,
//  и когда цена уже выставлена. Менять количество и убирать товар можно
...
import { toast, confirmDialog } from "../../ui.js?v=20260910a";
import { fmt } from "../../fx.js?v=20260910a";
import { идти } from "../app.js?v=20260910a";
import { мои } from "../api.js?v=20260910a";
import { картинка } from "../photo.js?v=20260910a";
import { счётчик } from "../stepper.js?v=20260910a";

const СОСТОЯНИЕ = {
  order:           { метка: "Считаем цену",   cls: "wait",  что: "Мы готовим цену. Пока можно поправить заказ." },
  pending_confirm: { метка: "Проверьте цену", cls: "ask",   что: "Цена готова. Можно поменять количество, убрать лишнее или добавить товары — и подтвердить." },
...
export default function render(box, ctx) {
  ...
  function подробно(o) {
    ...
    (o.items || []).forEach(it => {
      ...
      // выставлены: цена за штуку согласована, меняется только итог.
      // Заказ, который уже собирают, не трогаем.
      if (правится || спрашивают) {
    ...
    // Добавить товары можно на обоих шагах — через каталог и корзину.
    // Если цены уже выставлены, у нового товара цены нет, и заказ вернётся
    // к нам на расчёт: об этом честно предупреждаем заранее.
    if (правится || спрашивают) {
      box.append(el("button.btn.btn-outline", {
        style: { width: "100%", justifyContent: "center", minHeight: "46px", marginTop: "10px" },
        onclick: () => идти("catalog"),
      }, [icon("plus", { size: 16 }), "Добавить товары"]));
      if (спрашивают) {
        box.append(el("div.hint", { style: { marginTop: "6px" },
          text: "Добавите новый товар — мы посчитаем на него цену и пришлём заказ заново." }));
      }
    }
